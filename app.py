...
with st.sidebar:
    selected = option_menu("Main Menu", ["Add New captions/subtitles","Add New playlist" ,'View Stored Data','Classfy Stored Data','Visualize'], 
...
if selected =='Visualize':
    ...
    plass= st.multiselect('select class',['Environmental','Economic','Social','Cultural','Temporal','Developmental'])
    
    rules = {
                    'Environmental': ['natural','green','environmental','EnvlFriendly'],
                    'Economic':['saving','StandardOfLiving','efficiency','HouseholdNeeds'],
                    'Social':['Health','Well-being','Quality','Quantity','Diversity'],
                    'Cultural':['Tradition','Heritage'],
                    'Temporal':['Time','Long-termEffect'],
                    'Developmental':['Technology']
                            
                    }
    partOflist = []
    for i in plass:
        partOflist=partOflist+rules[i]
        
    partOflist
    df = pd.read_excel('classified_vidoes.xlsx')
    # df1 = df[pd.DataFrame(df.Categories.tolist()).isin(partOflist).any(1).values]
    # df = df[df['Categories'].str.contains('Ju')]
    pat = '|'.join(r"\b{}\b".format(x) for x in partOflist)
    df_plastic_prod = df[df['Categories'].str.contains(pat)]
    ...
    if plass:
        ...
        multiClass = pd.DataFrame()
        G =  nx.MultiGraph()
        G.add_node('Sustainability', size=15, title='root',color='blue',)
        print('==============')
        print(woedLen)
        print(len(word_frequencies.items()))
        ...
        for i in plass:
            G.add_edge('Sustainability',str(i))
            ...
            for word, frequency in word_frequencies.items():
                ...
                if word in rules[i] :
                    ...
                    G.add_edge(str(i),str(word)+":"+str(frequency))
                    G.add_node(str(word)+":"+str(frequency), size=10, title=str(word) +" frequency:"+str(frequency),color='green')
                    overClass = overClass + frequency
                    multiClass = multiClass.append({'root':'Sustainability','mainClass':str(i),'Class':str(word) , 'frequency': int(frequency) , 'ratio':int(frequency)/woedLen*100 }, ignore_index=True)
                    print(f"{word}: {frequency}")  
            G.nodes[i]['title'] = str(overClass)
            
        drug_net = Network(height='756px', width='756px',directed=True, bgcolor='white', font_color='black')
        drug_net.from_nx(G)
        drug_net.repulsion(node_distance=520, central_gravity=1.33,spring_length=110, spring_strength=1.70,damping=5.95,)
        try:
                path = 'img'
        ...
        st.title("classification by many Lables to each video")
        st.plotly_chart(figMulti)
        components.html(HtmlFile.read(), height=756 ,width=756,scrolling=True)
        fig = px.treemap(multiClass, path=['root','mainClass','Class', 'frequency'],
            values='frequency',
            color='mainClass')

        st.plotly_chart(fig)
        data=multiClass
        figall = px.pie(multiClass, values='frequency', names='mainClass', title='class ratio')
        st.plotly_chart(figall)
        for i in data.drop_duplicates(subset=['mainClass'])['mainClass']:
            dataframe=data[data['mainClass'].isin([str(i)])]
            # dataframe
            # dataframe= multiClass[multiClass['mainClass'].str.contains()]
            figx = px.pie(dataframe, values='frequency', names='Class', title=str(i))
    ...
    selected = st.selectbox('select class',['Environmental','Economic','Social','Cultural','Temporal','Developmental'])
    values = st.slider(
    ...
    G =  nx.MultiGraph()
    G.add_node('Sustainability', size=15, title='root',color='blue',)    
    G.add_edge('Sustainability',str(selected))   
    for x in Sustainability[selected]:
        
        print(x)
        
        G.add_edge(str(selected),str(x))
        for y in rules[x]:
            G.add_edge(str(x),str(y))
                    
    # drug_net.repulsion(node_distance=520, central_gravity=1.33,spring_length=110, spring_strength=1.70,damping=5.95,)                    
    drug_net = Network(height='665px', width='665px',directed=True, bgcolor='white', font_color='black',)
    drug_net.from_nx(G)
    drug_net.repulsion(node_distance=420, central_gravity=6.33,
                                    spring_length=110, spring_strength=1.10,
                                    damping=values)
    try:
            path = 'img'
            drug_net.save_graph(f'{path}\\pyvis_graph.html')
    ...
    selected = st.multiselect('select class ',['Environmental','Economic','Social','Cultural','Temporal','Developmental'])
    G =  nx.MultiGraph()
    G.add_node('Sustainability', size=15, title='root',color='blue',)    
    for i in selected:
        
        G.add_edge('Sustainability',str(i))   
        for x in Sustainability[i]:
            
            print(x)
            
            G.add_edge(str(i),str(x))
            for y in rules[x]:
                G.add_edge(str(x),str(y))
